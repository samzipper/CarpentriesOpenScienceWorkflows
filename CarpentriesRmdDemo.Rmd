...
We analyzed data from USGS gage `r usgs_id`. During the period of record, the maximum stage was `r max(data_clean$stage_ft)` and the minimum stage was `r min(data_clean$stage_ft)`. There were a total of `r sum(data_clean$stage_QAcode == "Gapfill")` missing dates that we gap-filled using linear interpolation.
